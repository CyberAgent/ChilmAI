...
1. 次のリンクをクリックして、最新の ZIP ファイルをダウンロードします
    - [ChilmAI-latest.zip をダウンロード](https://github.com/CyberAgent/ChilmAI/releases/latest/download/ChilmAI-latest.zip)
    - 配布ページ（[GitHub Releases](https://github.com/CyberAgent/ChilmAI/releases)）を開き、最新リリースの **Assets**（添付ファイル）から `ChilmAI-latest.zip` をクリックしてダウンロードすることもできます
2. ダウンロードした ZIP ファイルを展開します。Windows では次の手順で展開できます。
    1. エクスプローラーで、ダウンロードした `ChilmAI-latest.zip` を右クリックし、**「すべて展開」** を選択します
    2. 展開先フォルダを確認します。既定では ZIP と同じ場所に `ChilmAI-latest` フォルダが作られます（展開先は任意のフォルダに変更できます。例：`C:\Users\<ユーザー名>\Documents\`）
    3. **「展開」** をクリックします。展開先フォルダの中に、`ChilmAI.exe` を含む `ChilmAI-v0.1.5` のような、バージョン番号の付いたフォルダが作成されます
...
画面上部のロゴの横には、お使いの ChilmAI のバージョンが `v0.1.5` のような形式で表示されます。不具合の報告や問い合わせの際は、この番号をお知らせください。
...
??? note "IT 担当者向け補足"
    ...
    - `ChilmAI.exe` は PyInstaller の onedir ビルドです。展開してできたバージョン番号付きのフォルダの中に実行に必要なファイル一式がそのまま配置されており、起動時に Windows の一時フォルダへの展開は行われません。フォルダ内の他ファイルを削除・移動すると起動できなくなるため、フォルダごと移動・削除してください
