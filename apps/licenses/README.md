...
PEP 770 SBOM は同じバージョンでも OS ごとに異なるため、ビルド中にインストール済み wheel から読みます。`Cargo.lock` は target や feature を解決しないので、実際にコンパイルされるものより広くなります（`r-efi` は UEFI ターゲット専用で配布物には入りません）。正確な解決には cargo が必要なので、superset である旨を明記してそのまま出します。
...
- `ortools`: 同梱するコンパイル済みコードが Coin-OR（EPL-2.0）と Eigen（MPL-2.0）を静的リンクしています。wheel 自身の LICENSE（Apache-2.0）はライセンスファイルのコピーとして配られますが、静的リンクされた同梱物は Python のメタデータに現れないため、`BUNDLED_NATIVE_COMPONENTS` の一覧（取得元タグ・パッチ・上流条文）と `bundled-native-notices.txt` で別に開示します。EPL-2.0 / MPL-2.0 の条文全文は `SHARED_LICENSE_TEXTS` 経由で同梱します。
...
ortools v9.15 は Coin-OR の 5 コンポーネントを `coin-or/*` ではなく CMake 対応フォークの `Mizux/*` の `cmake/<version>` タグから取得し、`PATCH_COMMAND` でパッチを当ててからコンパイルします。Eigen も同様です。
...
EPL-2.0 の "Source Code" は configuration files を含むため、ビルド設定のみの変更も対応ソースとして扱います。v9.15 では EPL-2.0 / MPL-2.0 対象（Coin-OR 5 件と Eigen）のパッチはいずれも `CMakeLists.txt` のみの変更です。 `re2` と `SCIP` にもパッチが当たります。`pybind11_protobuf` はコミットハッシュに固定されています。`bzip2` は `GIT_TAG "master"` で取得されており上流にリビジョンが残らないため、固定できない旨をそのまま注記に出します（`unpinned=True`）。
...
strings -a ow/ortools/.libs/*.dll | grep -c constrained_cg
```
...
バンドルされる処理系はビルド環境によって異なります（CI は `actions/setup-python`、手元は uv 管理の python-build-standalone）。同じ 3.11 でもリンクされている OpenSSL のバージョンが異なり、バージョンを固定で書くと実際に配布する zip の中身と食い違うため、`runtime_components()` が実行中の処理系からバージョンを読み取ります。
...
この `LICENSE.txt` は OpenSSL・SQLite・libffi には触れていないため、この 3 つは上流のライセンス原文を `runtime/` に置いて配ります（`collect_native_notices.py` が取得します）。ライセンス原文は同じ系列の中では変わらないので取得元のタグは固定し、実際に同梱されるバージョンは一覧の側で処理系から読んで記載します。
...
| `verify_native_extensions_are_reviewed()` | 未確認、または確認済みバージョンから変わったネイティブ拡張 |
...
- `ortools`: 同梱物の構成もバージョンも変わります。後述の手順で確認し、 `BUNDLED_NATIVE_COMPONENTS`（取得元・パッチ・`notices`）と `bundled-native-notices.txt` を更新して `collect_native_notices.py` を再実行します。
- `pyinstaller`: ブートローダのバージョン。`PyInstaller-COPYING.txt` も取り直します。
- `pydantic-core` / `python-calamine`: `collect_rust_crate_notices.py` を再実行します。
- ネイティブ拡張を含む依存の増減・バージョン変更: 検査が止めるので、中身を確認して `NATIVE_REVIEWED_PACKAGES` のバージョンを書き換えます。
...
1. 取得元・タグ・パッチと、上流のビルド既定値を確認します。v9.15 では `USE_COINOR=ON`（EPL-2.0）、`USE_PDLP=ON`（→ `BUILD_Eigen3`、MPL-2.0）、 `USE_SCIP=ON`（→ SoPlex と Boost も同梱）、`USE_HIGHS=ON`、`USE_GLPK=OFF` です。

   ```bash
   BASE=https://raw.githubusercontent.com/google/or-tools/v9.15
   ...
   # 各パッチが触っている範囲
   for p in coinutils-2.11 osi-0.108 clp-1.17 cgl-0.60 cbc-2.10 eigen3-3.4.0; do
     echo "--- $p"
...
2. 配布対象である Windows wheel の実物を確認します。構成はプラットフォームとバージョンで違います。9.15 の Windows wheel は `ortools/.libs/` に共有 DLL を置く構成です： `ortools.dll`（Coin-OR と Eigen を静的リンク）、`libscip.dll`（SoPlex と Boost を静的リンク）、`highs.dll`、`libprotobuf.dll`、`libutf8_validity.dll`、 `abseil_dll.dll`、`re2.dll`、`zlib1.dll`、`bz2.dll`。
   ...
   # ortools.dll / libscip.dll に何が入っているか
   strings -a /tmp/ow/ortools/.libs/ortools.dll \
     | grep -cE 'CbcModel|ClpSimplex|OsiSolverInterface|CoinUtils|CglCutGenerator|Eigen'
   strings -a /tmp/ow/ortools/.libs/libscip.dll | grep -cE 'SoPlex|boost::'

   # 未リンクの根拠（GLPK と、Eigen の LGPL 対象 2 ファイル）
   for sym in glp_ constrained_cg IterationController; do
     printf '%s: %s\n' "$sym" "$(strings -a /tmp/ow/ortools/.libs/*.dll | grep -c "$sym")"
   done
   ```

   ChilmAI が呼び出すのは CP-SAT（`cp_model`）だけですが、その `.pyd` がロードする `ortools.dll` が Coin-OR と Eigen を含みます。EPL-2.0 §3.1 と MPL-2.0 §3.2 の義務は「配布しているかどうか」で決まるため、実際に呼び出しているかどうかにかかわらず開示が必要です。
...
3. 各コンポーネントのライセンスはそのバージョンのタグで確認します。Coin-OR は 2.10 前後で EPL-1.0 から EPL-2.0 に変わっています。ライセンス確認は上流タグでよいですが、開示するソース URL は実際に取得された `Mizux/*` の `cmake/*` タグです。フォークがライセンスを変えていないことも併せて確認します。
   ...
   ```bash
   curl -sSL https://raw.githubusercontent.com/coin-or/Cbc/releases/2.10.12/LICENSE | head -1
   curl -sSL https://raw.githubusercontent.com/Mizux/Cbc/cmake/2.10.12/LICENSE | head -1
   ```
